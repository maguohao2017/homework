var quit = {
    ...
    init:function(){
        ...
        var table = new BSTable(quit.id, "/quit/list_staff", defaultColunms);
    ...
    add:function(){
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/quit/quit_add_staff'
        });
    ...
    edit:function(){
        if (this.check()) {
            var index = layer.open({
                ...
                fix: false, //不固定
                maxmin: true,
                content: Feng.ctxPath + '/quit/quit_edit_staff/' + this.seItem.id
            });
            this.layerIndex = index;
        }
    },
    ...
    delete:function(){
        if (this.check()) {
            var operation = function(){
                ...
                var ajax = new $ax(Feng.ctxPath + "/quit/delete_staff/"+id, function () {
                    Feng.success("删除成功!");
                    quit.table.refresh();
                }, function (data) {
                    Feng.error("删除失败!");
