var travel = {
    ...
    init:function(){
        ...
        var table = new BSTable(travel.id, "/travel/list_staff", defaultColunms);
    ...
    add:function(){
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/travel/travel_add_staff'
        });
    ...
    edit:function(){
        if (this.check()) {
            var index = layer.open({
                ...
                fix: false, //不固定
                maxmin: true,
                content: Feng.ctxPath + '/travel/travel_edit_staff/' + this.seItem.id
            });
            this.layerIndex = index;
        }
    },
    ...
    delete:function(){
        if (this.check()) {
            var operation = function(){
                var id = travel.seItem.id;
                var ajax = new $ax(Feng.ctxPath + "/travel/delete_staff/"+id, function () {
                    Feng.success("删除成功!");
